...
CMD="$1"
IMAGE_NAME="${IMAGE_NAME:-ver/verilocale-backend}"
REMOTE="${REMOTE:-root@159.138.228.40}"
SSH_KEY="${SSH_KEY:-$HOME/.ssh/KeyPair-v2.pem}"
REMOTE_DIR="${REMOTE_DIR:-/root/gk/backend}"
TS=$(date +%Y%m%d-%H%M%S)
...
send_files() {
  ...
  scp -i "$SSH_KEY" -r config.dev.yaml "$REMOTE:$REMOTE_DIR/"
  # optional backend runtime config
  [ -f config.prod.yaml ] && scp -i "$SSH_KEY" -r config.prod.yaml "$REMOTE:$REMOTE_DIR/config.yaml"
}
...
prune_local_tars() {
  ...
  keep="$2"
  [ -d "$dir" ] || return 0
  count=$(ls -1t "$dir"/*backend*.tar.gz 2>/dev/null | wc -l | tr -d ' ')
...
dev)
  ...
  cp ./docs/public/swagger.json ~/Downloads/verilocale---ai-e-kyc-platform-1/res/docs
  ...
  health_wait "verilocale-backend-dev" 10
  ;;
prod)
  ...
  OUT_BASE="$(basename "$OUT")"
  REMOTE_OUT="$REMOTE_BASE/$OUT_BASE"
  ./scripts/gen-swagger-public.sh
  cp ./docs/public/swagger.json ~/Downloads/verilocale---ai-e-kyc-platform-1/res/docs
  docker buildx build --platform linux/amd64 -t "$IMG" .
  docker save "$IMG" | gzip >"$OUT"
  send_files "$OUT"
  remote "mkdir -p $REMOTE_BASE/backups"
  remote "cur=\$(docker inspect -f '{{.Config.Image}}' verilocale-backend 2>/dev/null || echo '$IMAGE_NAME:1.0.0'); tag=\$(echo \"\$cur\" | awk -F: '{print \$2}'); docker save '$IMAGE_NAME:'\$tag | gzip > $REMOTE_BASE/backups/${IMAGE_NAME##*/}_\$tag.tar.gz; echo \$tag > $REMOTE_DIR/.prod_prev || true"
  remote "docker load -i $REMOTE_OUT"
  ...
  remote "cd $REMOTE_DIR; PROD_TAG=$PROD_TAG_IN docker compose up -d kyc-service"
  health_wait "verilocale-backend" 60
...
push)
  ...
  ;;
rollback)
  remote "cd $REMOTE_DIR; prev=\$(cat .prod_prev 2>/dev/null || echo 1.0.0); PROD_TAG=\$prev docker compose up -d kyc-service"
  health_wait "verilocale-backend" 60
  ;;
*)
  echo "Usage: ./build.sh [dev|prod|promote|rollback]"
