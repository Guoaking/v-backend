...
func Init(level string) {
    ...
    if v := os.Getenv("KYC_LOG_MAX_SIZE"); v != "" {
        if i, err := strconv.Atoi(v); err == nil { maxSize = i }
    }
    maxBackups := 7
    if v := os.Getenv("KYC_LOG_MAX_BACKUPS"); v != "" {
        if i, err := strconv.Atoi(v); err == nil { maxBackups = i }
    }
    maxAge := 14
    if v := os.Getenv("KYC_LOG_MAX_AGE"); v != "" {
        if i, err := strconv.Atoi(v); err == nil { maxAge = i }
    }
    compress := true
    if v := os.Getenv("KYC_LOG_COMPRESS"); v != "" {
        if b, err := strconv.ParseBool(v); err == nil { compress = b }
    }
