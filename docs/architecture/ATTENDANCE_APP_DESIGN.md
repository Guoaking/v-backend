...
2. **打卡模式动态调整**：允许管理员在 Console 动态调整前端的打卡要求。例如，初期员工抵触人脸识别，可配置为“仅拍照打卡”（跳过活体，状态直接置为 `manual_review`）。后续可无缝切换为“静默活体”或“动作活体”。前端 H5 通过 `/api/v1/attendance/config` 接口获取该配置并动态渲染。
