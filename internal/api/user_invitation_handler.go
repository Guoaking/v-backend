...
func NewUserInvitationHandler(svc *service.KYCService) *UserInvitationHandler {
	return &UserInvitationHandler{service: svc}
}
...
func (h *UserInvitationHandler) AcceptMyInvitation(c *gin.Context) {
	id := c.Param("id")
	if id == "" {
		JSONError(c, CodeMissingParameter, "缺少邀请ID")
		return
	}
	email := c.GetString("userEmail")
